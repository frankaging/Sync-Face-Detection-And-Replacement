...
%% Warning!:
% 1. Please Modify the Path Before Futher Run
% 2. Please Modify the Face Num You Want To Replace
% 3. Set Up A Diff Threshold By Hand For Different Videos(This parameters 
%    is used to ensure the face landmark on the sequences of frames are 
%    corresponding to the same face(s))
...
len_dest_faces =1; % num is the max number of faces in a video
...
%% Motion Compensation And Morphing
for i=1:num; %loop for every frame 
    ...
    if size(landmark_dest_pre,1) == size(landmark_dest_current,1) && size(landmark_dest_current,1) == size(landmark_dest_follow,1) && size(landmark_dest_prepre,1) == size(landmark_dest_pre,1) && size(landmark_dest_follow,1) == size(landmark_dest_followfollow,1)
        ...
        % the same, and also making sure they are detecting the same face(s)
        ...
        % matrix built for examing if the face(s) is/are the same faces in
        % those frames
        m1 = [landmark_dest_preprer(:,1);landmark_dest_prer(:,1);landmark_dest_currentr(:,1);landmark_dest_followr(:,1);landmark_dest_followfollowr(:,1)];
        m2 = [landmark_dest_preprer(:,1);landmark_dest_preprer(:,1);landmark_dest_prer(:,1);landmark_dest_currentr(:,1);landmark_dest_followr(:,1)];
        
        diff = abs(sum(m1-m2));
        
        % this threshold needs to be set-up based on the testing video
        if diff < 600
